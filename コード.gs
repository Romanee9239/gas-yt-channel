...
function getMovieData () {
  ...
  var book = SpreadsheetApp.getActiveSpreadsheet();
  var sheetData = book.getSheetByName("動画分析");
  ...
  */
  //key.pri = key_arr[0];
  key.pri = 'AIzaSyC5yoDsNzrbRN8HIYMDYNEVg1g8RoZbEOo';
  ...
  */
  //https://www.youtube.com/channel/UCPeBnn5xaNqtHvAHXrCp19Q/
  
  //var id = cid_arr[0];
  
  var id = 'UCPeBnn5xaNqtHvAHXrCp19Q';
  ...
  */
  ...
  Browser.msgBox(c_contentDetails.relatedPlaylists.uploads);
...
end_label:
  for(var j = 0; j<(total/50); j+=1){
    ...
    for(var k = count; k<count + list.length; k+=1){
      ...
      var vprm = "&part=snippet,contentDetails,statistics,suggestions&id=" + targetVideoId[k];
      ...
      //推奨タグ
      Browser.msgBox(data.suggestions);
      //targetSuggestedTags[k] = data.suggestions.tagSuggestions[];
    }
    count = k;
  }
  
  //チャンネルのシート取得
  var newSheet = book.getSheetByName("動画分析");
  if(newSheet != null){
